...
export function MbTwoFA ( {hide}: Props) {
	...
	const handle2faOn = async () => {
		try {
			const response = await axios.post(`http://${import.meta.env.VITE_API_UR}/2fa/turn-on`, code, { withCredentials: true })
		...
		const handle2faOff = async () => {
			try {
				const response = await axios.post(`http://${import.meta.env.VITE_API_UR}/2fa/turn-off`, code, { withCredentials: true })
				.then (function (response) {
					Setsent(true);
		...
		try {
			const response = await axios.get(`http://${import.meta.env.VITE_API_UR}/2fa/generate-qrcode`, {
	...
	const handleOff = () => {
	  ...
	  useEffect(() => {
		  try {
			const response =  axios.get(`http://${import.meta.env.VITE_API_UR}/profile/me`, { withCredentials: true })
			.then((respnse) => {
			})
		  } catch (error) {
