...
export function DkSettings ( {hide}: Props ) {
	...
	const data = useContext(UserContext);
	  ...
	  useEffect(() => {
		  ...
		  try {
			const response =  axios.get(`http://${import.meta.env.VITE_API_UR}/profile/me`, { withCredentials: true }).then ( function(response) {
	...
	const handleFileUpload = async (event: any) => {
		try {
			...
			axios.post(`http://${import.meta.env.VITE_API_UR}/upload-avatar`, formData,  {
			...
			.then((response) => {
				...
				setBase(`http://${import.meta.env.VITE_API_UR}/avatars/${response.data}`);
	...
	const handleName = async () => {
		try {
			...
			const response = await axios.post(`http://${import.meta.env.VITE_API_UR}/set-username`, formData, {withCredentials: true}).then (function (response) {
				console.log(response.data);
				data?.setUserData((prevUserData) => ({
					...prevUserData,
					user_data: {
