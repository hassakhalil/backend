...
export function HeadProfile ( {profile, name, friendNum, me}: Props ) {
	...
	const handleFriend = async () => {
		try {
			const response = await axios.post(`http://${import.meta.env.VITE_API_UR}/add-friend/${name}`, null, { withCredentials: true })
	...
	const blockFriend = async () => {
		try {
			const response = await axios.post(`http://${import.meta.env.VITE_API_UR}/block-friend/${name}`, null, {withCredentials: true});
			console.log("out");
		}
		catch (error) {
			console.log(error);
